...
function NavigationPanelComponent() {
  ...
  const getSurveyLink = () => {
    return `${isEnvSet("VITE_SURVEY_LINK")}&${isEnvSet(
    ...
    >
      {store.graphInstance.hoverData.map((entry, index) => (
        <Box width="100%" key={`hover_data_${index}`}>
          <Text
            fontSize="xs"
            color={colorMode === "light" ? "blackAlpha.800" : "gray.400"}
            fontWeight="bold"
            overflow="hidden"
            whiteSpace="nowrap"
  ...
  return (
    <>
      <Box
        ...
        height="60px"
        backgroundColor={
          location.pathname !== "/" && location.pathname !== "/present"
            ? bgColor
            : "transparent"
        }
        right="0px"
...
const ObservedNavigationPanelComponent = observer(NavigationPanelComponent);
export default ObservedNavigationPanelComponent;
