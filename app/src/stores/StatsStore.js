...
export class StatsStore {
  ...
  getGraphColors = (labels, data) => {
    ...
    if (data && uniqueValues && uniqueValues?.length <= 10) {
      for (let i = 0; i < labels.length; i++) {
        graphColors.push(schemeTableau10[uniqueValues.indexOf(data[i])]);
      }
    } else {
      for (let i = 0; i < labels.length; i++) {
        if (labels.length <= 10) {
  ...
  setWidgetProperty = (widgetID, property, value) => {
    this.activeWidgets.find((chart) => chart.id === widgetID)[property] = value;
  ...
  addChart = () => {
    ...
    const charts = this.store.stats.charts[this.store.search.currentDataset];
  ...
  getWidgetItemCount = (id) =>
    this.activeWidgets.find((widget) => widget.id === id)?.display_limit || 10;
  ...
  getRadarDetailComponents = () => {
    ...
    const datasets = this.store.graph.currentGraphData.selectedComponents.map(
      (componentID) => {
    ...
    return properties.map((linkType) => {
      return nodeLinks.reduce((counts, link) => {
        link.connections.forEach((connection) => {
          counts = connection.feature === linkType ? counts + 1 : counts;
        });
        return counts;
      }, 0);
    });
          ...
          data: [
            ...Object.keys(visibleProperties)
              .map((key) => {
                ...
                return this.getAnchorProperties(node, visibleProperties[key]);
              })
              .flat(),
          ],
        };
  ...
  getEdgeAllValueCounts = (
    ...
    if (edgeProperty.type === "basic") {
      data.forEach((link) => {
        allValueCounts[uniqueValues.indexOf(link[edgeProperty.prop])] += 1;
      });
    } else {
      data.forEach((link) => {
        link.connections.forEach((connection) => {
          allValueCounts[uniqueValues.indexOf(connection[edgeProperty.prop])] +=
            1;
        });
      });
    }
  ...
  getEdgeCounts = (
    ...
    if (groupBy) {
      ...
      Object.keys(groups).forEach((group) => {
        ...
        if (edgeProperty.type === "basic") {
          data.forEach((link) => {
            const labelLocation = valuesSorted.indexOf(link[edgeProperty.prop]);
    ...
    if (edgeProperty.type === "basic") {
      ...
      data.forEach((link) => {
        if (link.connections) {
          link.connections.forEach((connection) => {
            const labelLocation = values.indexOf(connection[edgeProperty.prop]);
    ...
    return {
      edgeProperty: edgeProperty,
      ...this.getChartDataBasedOnType(valuesSorted, countsSorted, chartType),
    };
  };
  ...
  getNodeAdvancedProp = (node, prop) =>
    Object.hasOwn(node, "properties") && Object.hasOwn(node.properties, prop)
  ...
  getNodeCounts = (
    ...
    let data = this.getNodeDataBasedOnNetworkSelection(network_data, show_only);
    ...
    if (groupBy) {
      ...
      // Unique node values
      const uniqueValues = [
        ...new Set(data.map((node) => getNodeProp(node, nodeProperty.prop))),
      ];
      ...
      const visibleEntries =
        network_data === "visible"
          ...
          : this.store.graph.currentGraphData.tableData.map((row) => row.entry);
      ...
      data
        ...
        .forEach((entry) => {
          allValueCounts[uniqueValues.indexOf(entry.value)] += entry.weight;
        });
      ...
      Object.keys(groups).forEach((group) => {
        ...
        data.forEach((node) => {
          ...
          groupedByCounts[group][labelLocation] += node.entries.filter(
            (entryID) => visibleEntries.includes(entryID)
          ).length;
        });
      });
      ...
      data.forEach((node) => {
        ...
        if (sortBy === "frequency") {
          if (labelLocation >= 0) {
            ...
            counts.push(
              node.entries.filter((entryID) => visibleEntries.includes(entryID))
                .length
            );
          }
          ...
          if (labelLocation >= 0) {
            counts[labelLocation] += parseFloat(node.properties[sortBy]);
          } else {
            values.push(getNodeProp(node, nodeProperty.prop));
