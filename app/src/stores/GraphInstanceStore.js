...
export class GraphInstanceStore {
  ...
  setPanSpeed = (speed) => {
    ...
    const panningSpeedGraphValues = [0.005, 0.01, 0.05, 0.1, 0.3, 0.5, 1, 2, 3];
  ...
  triggerSelectedNodes = () => {
    ...
    const selectedNodeIds = this.store.graph.currentGraphData.selectedNodes.map(
      (node) => node.id
    );

    // Find links that are connecting selected nodes
    for (let i = 0; i < linkCount; i++) {
      let linkSource = this.store.graph.currentGraphData.links[i].source.id;
      let linkTarget = this.store.graph.currentGraphData.links[i].target.id;
  ...
  filterNodesByNumericProp = (min, max, prop) => {
    ...
    if (prop === "degree") {
      for (let i = 0; i < nodeCount; i++) {
        if (
          this.store.graph.currentGraphData.nodes[i].neighbours.size === 0 &&
          !this.store.graphInstance.orphanNodeVisibility
        ) {
          this.store.graph.currentGraphData.nodes[i].visible = false;
        } else {
          this.store.graph.currentGraphData.nodes[i].visible =
            this.store.graph.currentGraphData.nodes[i].neighbours.size >= min &&
            this.store.graph.currentGraphData.nodes[i].neighbours.size <= max;
        }
      }
    } else {
      for (let i = 0; i < nodeCount; i++) {
        if (
          ...
          this.store.graph.currentGraphData.nodes[i].visible =
            parseFloat(
              this.store.graph.currentGraphData.nodes[i].properties[prop]
            ) >= min &&
            parseFloat(
              this.store.graph.currentGraphData.nodes[i].properties[prop]
            ) <= max;
    ...
    for (let i = 0; i < nodeCount; i++) {
      if (this.store.graph.currentGraphData.nodes[i].visible) {
        visibleNodeIds.push(this.store.graph.currentGraphData.nodes[i].id);
      }
    }
  ...
  triggerMultiSelfCentric = (onlyMutual = false, mutualWithOrigin = false) => {
    const selectedNodeIds = this.store.graph.currentGraphData.selectedNodes.map(
      (node) => node.id
    );
    ...
    visibleNodeIdsWithoutMain = Array.from(new Set(visibleNodeIdsWithoutMain));
    ...
    for (let i = 0; i < nodeCount; i++) {
      this.store.graph.currentGraphData.nodes[i].visible =
        visibleNodeIds.includes(this.store.graph.currentGraphData.nodes[i].id);
    }
  ...
  triggerSameEntry = (withSelectedNodes = false) => {
    ...
    if (withSelectedNodes) {
      visibleNodeIds = this.store.graph.currentGraphData.selectedNodes.map(
        (node) => node.id
      );
  ...
  generateSchemeColorsFromArray = (values, feature, schemeType = "node") => {
    const skipfactor = values && values.length > 10 ? 1 / values.length : null;
      ...
      if (min === max) {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          if (norm_values[i] > 0.9) {
            ...
            interpolated = interpolateYlOrRd(norm_values[i]).match(/[0-9]+/g);
          }
  ...
  toggleVisibleComponents = (componentId) => {
    ...
    this.store.graph.currentGraphData.selectedNodes.forEach((node, index) => {
    ...
    if (componentId === -1) {
      ...
      const componentEntries = [
        ...new Set(
          selectedComponents.map((component) => component["entries"]).flat()
        ),
      ];
  ...
  filterEdgesByMinMaxVal = (min, max) => {
    ...
    for (let i = 0; i < linkCount; i++) {
      ...
      if (isVisible) {
        visibleIds.push(this.store.graph.currentGraphData.links[i].source.id);
        visibleIds.push(this.store.graph.currentGraphData.links[i].target.id);
      }
    }
  ...
  filterEdgesWithValue = (property, value) => {
    ...
    if (property.type === "basic") {
      for (let i = 0; i < linkCount; i++) {
        let isVisible =
          this.store.graph.currentGraphData.links[i][property.prop] === value;
        this.store.graph.currentGraphData.links[i].visible = isVisible;
        if (isVisible) {
          visibleIds.push(this.store.graph.currentGraphData.links[i].source.id);
          visibleIds.push(this.store.graph.currentGraphData.links[i].target.id);
      ...
      for (let i = 0; i < linkCount; i++) {
        let isVisible = this.store.graph.currentGraphData.links[
          i
        ].connections.find((connection) => connection[property.prop] === value);
