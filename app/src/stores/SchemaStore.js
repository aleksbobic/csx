...
export class SchemaStore {
  ...
  getServerSchema = () => {
    return this.edges.map((edge) => {
      ...
      let serverRelationship = this.relationshipMapping[edge.data.relationship];
  ...
  generateSchemaNode = (label) => {
    ...
    return {
      ...
      data: {
        label: label,
        isVisible: this.store.core.visibleDimensions["detail"].includes(label),
        toggleVisibility: this.toggleVisibility,
      },
      style: {
        background: this.store.core.visibleDimensions["detail"].includes(label)
          ? "#283b57"
          : "#323232",
        color: "white",
        borderRadius: "10px",
  ...
  generateLink = (link) => {
    ...
    const possibleRelations = this.getPossibleRelations(link.src, link.dest);
  ...
  loadDefaultSchema = (id) => {
    ...
    const schema_nodes = [
      ...new Set(
        schema_to_load.edges.map((edge) => [edge.source, edge.target]).flat()
      ),
    ].map(
      (node_id) =>
        schema_to_load.nodes.find((node) => node.id === node_id).data.label
    );
      ...
      Object.keys(this.store.fileUpload.fileUploadData.defaults).forEach(
        (feature) => {
          this.featureTypes[feature] =
            this.store.fileUpload.fileUploadData.defaults[feature].dataType;
        }
      );
  ...
  refreshNodeStyles = () => {
    this.nodes = [
      ...this.nodes.map((node) => {
        ...
        node.style = {
          ...node.style,
          background: this.store.core.visibleDimensions["detail"].includes(
            node.data.label
          )
            ? "#283b57"
            : "#323232",
