...
export class SearchStore {
  ...
  deleteDataset = async (dataset) => {
    const { error } = await safeRequest(axios.delete(`datasets/${dataset}`));
    ...
    this.store.core.setToastMessage(
      `${dataset.charAt(0).toUpperCase()}${dataset.slice(1)} dataset deleted 🙂`
    );
  ...
  getConifg = async (dataset) => {
    ...
    this.store.fileUpload.populateDataFromConfig(dataset, response.data.config);
  };

  suggest = async (feature, input) => {
    if (this.datasetTypes[this.currentDataset] === "api" && input.length < 3) {
      return [input];
    }
