...
export class PresentStore {
  ...
  handleUnorderedList = (
    ...
    const newNextOffset =
      this.getInchesFromPoints(this.getPointsFromPixels(13 * numberOfLines)) +
    ...
    if ((index || slide.align) && slide.screenshot) {
      if (slide.align) {
        if (slide.align === "left") {
          ...
          textWidthBasedOnOffset = `${
            10 - 3.5 * (slide.screenshotWidth / slide.screenshotHeight) - 0.2
          }`;
        } else {
          xOffset = `${
            10 - 3.5 * (slide.screenshotWidth / slide.screenshotHeight) - 0.2
        ...
        xOffset =
          index % 2
            ? "0%"
            : `${10 - 3.5 * (slide.screenshotWidth / slide.screenshotHeight)}`;
    ...
    contentChunks.forEach((row, index) => {
      if (/^#{1,6} /.test(row)) {
          ...
          this.getInchesFromPoints(
            this.getPointsFromPixels(
              this.getTextWidth(
                ...
                `${this.getPointsFromPixels(
                  headingPixelSizes[row.split(" ")[0].length - 1] + 5
                )}pt arial`
              )
            )
          ) / textWidthBasedOnOffset
        ...
        slidesWithOffsets.push({
          ...
          height: this.getInchesFromPoints(
            this.getPointsFromPixels(
              headingPixelSizes[row.split(" ")[0].length - 1] * numberOfLines
            )
          ),
  ...
  getParentIDs = (id, history) => {
    ...
    if (focusItem.parent !== "None") {
      return [focusItem.id, ...this.getParentIDs(focusItem.parent, history)];
    } else {
      return [focusItem.id];
    }
  };
    ...
    sections.forEach((entry) => {
      if (entry.section !== "1") {
        ...
        orderedSections.splice(
          parentLocation + 1 + orderedSections[parentLocation].children,
          0,
          {
            ...entry,
            children: 0,
  ...
  generateSlides = async (studyID, publicStudyID, activeItem) => {
    ...
    studyDetails.history.forEach((item) => {
      const children = this.getDirectChildren(item.id, studyDetails.history);
        ...
        if (entry.comments.length > 0) {
          entry.comments.forEach((comment) => {
            if (comment.chart && comment.screenshot) {
              const align = sectionSlides.length % 2 ? "left" : "right";
              sectionSlides.push({
                type: "markdownmedia",
