...
function ConnectionStats({
    ...
    return Object.keys(neighbour_counts)
      ...
      .map((feature, index) => (
        <Tag
          ...
          background={
            demoData.length === 0 && store.core.currentGraph === "detail"
      ...
      >
        {isExpanded && (
          <HStack width="100%">
            ...
            <Tooltip
              label={`Show all nodes with selected feature ${level + 1} ${
        ...
        <Wrap width="100%">
          <Tag
            borderRadius="6px"
            backgroundColor={colorMode === "light" && "blackAlpha.300"}
          >
            <Text color="whiteAlpha.900" fontSize="xs" fontWeight="bold">
  ...
  const renderNodeDetails = (node) => {
    ...
    for (
      ...
      neighborDegreeLevels[degreeLevel - 1].neighbours = results.newNeighbors;
    ...
    return (
      <>
        {neighborDegreeLevels
          ...
          .map((level, index) => {
            if (index === 0 || (isExpanded && level.neighbours.length > 0)) {
              return renderDetailLevel(node, neighborDegreeLevels, index);
            }
            return null;
          })}
      </>
