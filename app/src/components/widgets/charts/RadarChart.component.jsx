...
function RadarChart({
  ...
  const [tooManySelectedElements, setTooManySelectedElements] = useState(false);
      ...
      if (chart.elements === "nodes") {
        if (store.core.isDetail) {
          const nodeProperties =
            store.stats.getRadarDetailVisibleNodeProperties(chart.id);

          data = store.stats.getRadarDetailNodes(nodeProperties);
        } else {
          const nodeProperties =
            store.stats.getRadarOverviewVisibleNodeProperties(chart.id);
  ...
  const getPluginOptions = () => {
    ...
    switch (chartConfig.element_values) {
      ...
      case "types":
        propsInChart =
          chartConfig.elements === "nodes" ? "Node feature" : "Edge feature";
        break;
      default:
        propsInChart =
          chartConfig.elements === "nodes"
