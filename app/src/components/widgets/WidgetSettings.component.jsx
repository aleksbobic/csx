...
function WidgetSettings(props) {
  ...
  useEffect(() => {
    setAvailableNodeProperties(store.stats.getWidgetAvailabelNodeProperties());
  ...
  const renderTitle = (index) => (
    <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
      ...
      <Editable
        ...
        onSubmit={(val) => {
          if (val.trim()) {
            store.stats.setWidgetProperty(props.widgetID, "title", val.trim());
            setTitle(val.trim());
            ...
            store.track.trackEvent(
              ...
              },
              {
                event_type: "Write",
                event_action: "Change title",
                event_value: store.stats.getWidgetTitle(props.widgetID),
              }
            );
          }
        }}
      ...
      <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
        ...
        <Select
          ...
          onChange={(e) => {
            ...
            store.stats.setWidgetProperties(props.widgetID, {
              ...
              element_sort_values:
                newItemType !== "nodes" ? "frequency" : secondaryAxisValue,
            });
        ...
        >
          {availableTypes.map((entry) => (
            <option key={`item_type_${entry.value}`} value={entry.value}>
              {entry.label}
            </option>
          ))}
        </Select>
  ...
  const renderMainAxisItemSwitch = (index) => (
    <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
      <Heading size="xs" opacity="0.5" width="100%">
        {props.mainAxis ? `${props.mainAxis} Axis Props` : "Widget Props"}
      </Heading>
      <Select
        className="nodrag"
        margin="0px"
  ...
  const renderSecondaryAxisItemSwitch = (index) => (
    <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
      ...
      <Select
        className="nodrag"
        isDisabled={itemType === "edges" || selectedItemProp !== "values"}
        margin="0px"
        variant="filled"
        size="xs"
        width="100%"
  ...
  const renderItemStateSwitch = (index) => {
    ...
    if (props.customItemStates) {
      ...
      itemStates = [{ value: "visible", label: "Visible graph elements" }];
      if (itemType !== "edges") {
        itemStates.push({
          value: "selected",
      ...
      <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
        ...
        >
          {itemStates.map((state) => (
            <option key={`item_state_${state.value}`} value={state.value}>
              {state.label}
            </option>
          ))}
        </Select>
      ...
      <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
        ...
        >
          ...
          {[...store.graph.currentGraphData.perspectivesInGraph].map(
            (entry) => (
              <option key={`chart_settings_show_only_${entry}`} value={entry}>
  ...
  const renderVisibleNodeProperties = (index) => {
    return (
      <VStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
        <Heading size="xs" opacity="0.5" width="100%">
          Visible Node Props:
        </Heading>
        <VStack
        ...
        >
          {availableNodeProperties.map((entry) => (
            <Checkbox
              defaultChecked={visibleNodeProperties.includes(entry)}
              isDisabled={
              ...
              onChange={(event) => {
                if (event.target.checked) {
                  setVisibleNodeProperties([...visibleNodeProperties, entry]);
                  ...
                  setVisibleNodeProperties([...visibleNodeProperties]);
  ...
  const renderDirectConnectionFeatureSwitch = (index) => (
    <HStack width="100%" key={`${props.widgetID}_settings_widget_${index}`}>
      ...
      >
        <option value="all">all</option>
        {store.graph.currentGraphData.perspectivesInGraph.map((feature) => (
          <option key={`connection_feature_${feature}`} value={feature}>
            {feature}
          </option>
        ))}
      </Select>
...
const ObservedWidgetSettings = observer(WidgetSettings);
export default ObservedWidgetSettings;
