  ...
  const renderInteractionTracking = () => (
    <Box>
      ...
      <Text
        ...
        color={textColor}
      >
        To further improve CSX and provide new and exciting features, we would
        like to ask you to enable interaction tracking. As you can see from the
        above-provided information, we do not store user-identifiable
        information. We will use the collected data for two purposes:
      </Text>
      ...
      >
        <ListItem>
          ...
          />
          Improving CSX with new features (this may also include using the data
          to train machine learning features such as recommender systems) and
          fixing bugs.
        </ListItem>
        <ListItem>
          <ChevronRightIcon
          ...
          />
          Contributing to research through interaction data analysis and also
          through potentially open-sourcing the collected data.
        </ListItem>
      </List>
      <Text
        fontSize="xs"
      ...
      >
        ...
        . This will allow us to collect additional interaction data with CSX
        (such as button clicks and page navigation) in addition to the data
        mentioned above and enable us to analyze the data associated with your
        unique ID for the purposes stated above. The data will be stored only
        for the duration needed to conduct analysis and will be deleted after.
        It may be published in fully anonymised form in scientific publications.
        During the collection phase the data is stored on a server located in
        the European Union. If for any reason you decide to opt-out of the
        interaction tracking, you can do so by clicking the button below. Please
        note that this will not delete any of the data that has already been
        collected immediatelly but the data will not be used for analysis and
        will be deleted at the earliest possible time.
      </Text>
      ...
      >
        ...
        <b style={{ color: "#43a2fb" }}>will add additional cookies</b>. You can
        read about Matomo&apos;s cookies by visiting the{" "}
        <Link
          fontWeight="bold"
      ...
      <FormControl display="flex" alignItems="center" paddingLeft="16px">
        ...
        <Text
          ...
          color={textColor}
        >
          Tracking {store.core.trackingEnabled ? "enabled" : "disabled"}
        </Text>
      </FormControl>
    </Box>
  );
  ...
  return (
    ...
    >
      ...
      <Text
        ...
        color={textColor}
      >
        You might be wondering does Collaboration Spotting X use cookies and
        what is their purpose. In short, CSX does not use cookies by default.
        However, it does use local storage to store a multitude of values
        necessary to provide various features such as studies, multiple
        datasets, local settings storage, and more without demanding that you
        log in. Here you can find a short description of all local storage
        values and their purpose. Since storing these values is necessary for
        the system&apos;s proper functioning, these values are saved
        automatically. By using CSX, you also understand and agree that data
        such as your comments in studies and the study history have to be stored
        in our database.
      </Text>
      ...
      <Box padding="16px" width="100%" paddingBottom="0px" paddingTop="0px">
        <Table>
          <Thead>
            <Tr>
              <Th
              ...
              <Th padding="8px" borderBottom="none" color={textColor}>
                Purpose
              </Th>
            </Tr>
          </Thead>
      ...
      {isEnvFalse("VITE_DISABLE_TRACKING") && renderInteractionTracking()}
    </VStack>
  );
}
...
const ObservedCookieInfo = observer(CookieInfo);

export default ObservedCookieInfo;
