...
function GraphStats(props) {
  ...
  const getNodeData = useCallback(
    (widget) => {
      ...
      switch (widget?.network_data) {
        ...
        case "selected":
          node_counts = store.graph.currentGraphData.selectedNodes.reduce(
            (counts, node) => {
              if (Object.keys(counts).includes(node.feature)) {
                counts[node.feature] += 1;
              } else {
        ...
        default:
          return Object.entries(store.graph.currentGraphData.types).map(
            (entry) => [entry[0], { count: entry[1].count, label: entry[0] }]
          );
      }
    },
    [
  ...
  const renderStatsGroup = (data, title) => (
    <VStack width="100%">
      <Heading
        ...
        paddingLeft="4px"
        size="xs"
        color={colorMode === "light" ? "blackAlpha.500" : "whiteAlpha.300"}
        marginBottom="4px"
      >
        {title}
      </Heading>
  ...
  if (props.settingsMode && props.isExpanded) {
    return (
      <WidgetSettings widgetID={props.chart.id} settings={["item state"]} />
    );
  }
