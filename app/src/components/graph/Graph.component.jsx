  ...
  const onNodeHover = (node, nodeout) => {
    ...
    if (node) {
      if (
        ...
        const contextCoordinates = containerRef.current.graph2ScreenCoords(
          node.x,
          node.y,
          node.z ? node.z : 0
        );
    ...
    (node) => {
      ...
      if (
        ...
        (store.graphInstance.labels.labelFeatures.length === 0 ||
          store.graphInstance.labels.labelFeatures.includes(node.feature))
      ) {
  ...
  return (
    <ForceGraph3D
      ...
      linkDirectionalArrowLength={
        store.core.isDetail && store.graphInstance.edgeDirectionVisiblity
          ? 10
          : 0
      }
